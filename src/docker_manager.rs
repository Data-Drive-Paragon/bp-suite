...
fn check_power_status() {
                ...
                if type_content.trim() == "Battery" {
                    ...
                    if let Ok(status) = std::fs::read_to_string(&status_path) {
                        if status.trim() == "Discharging" {
                            ...
                            log::warn!(
                                "Docker builds may take significantly longer and could drain battery quickly."
                            );
        ...
        for entry in thermal_dir.flatten() {
            ...
            if let Ok(temp_str) = std::fs::read_to_string(&temp_path) {
                if let Ok(temp_millidegrees) = temp_str.trim().parse::<i32>() {
                    ...
                    if temp_celsius > 80 {
                        log::warn!(
                            "⚠️  WARNING: High temperature detected ({}°C)",
                            temp_celsius
                        );
        ...
        std::fs::read_to_string("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor")
    {
        if governor.trim() == "powersave" || governor.trim() == "conservative" {
            ...
            log::warn!(
                "Performance may be limited. Consider switching to 'performance' mode for faster builds."
            );
        }
    }
}
...
pub fn start_services(strict: bool, internal_tailscale: bool, no_rebuild: bool) -> Result<()> {
    ...
    // Load connector.toml
    let connector_content =
        std::fs::read_to_string("connector.toml").context("Failed to read connector.toml")?;
    let connector_config: ConnectorConfig =
        toml::from_str(&connector_content).context("Failed to parse connector.toml")?;

    // Load pool.toml
    let pool_content = std::fs::read_to_string("pool.toml").context("Failed to read pool.toml")?;
    let pool_config: PoolConfig =
        toml::from_str(&pool_content).context("Failed to parse pool.toml")?;
...
fn start_postgres_nodes(
    ...
    let nodes = [
        (
            "postgres_node_1",
            &connector.postgres_node_1,
            &pool.postgres_node_1,
        ),
        (
            "postgres_node_2",
            &connector.postgres_node_2,
            &pool.postgres_node_2,
        ),
        (
            "postgres_node_3",
            &connector.postgres_node_3,
            &pool.postgres_node_3,
        ),
        (
            "postgres_node_4",
            &connector.postgres_node_4,
            &pool.postgres_node_4,
        ),
    ];
    ...
    for (name, connector_node, pool_conn) in nodes {
        if let (Some(node), Some(conn)) = (connector_node, pool_conn) {
            if !conn.enable {
                log::info!("Skipping disabled PostgreSQL node: {}", name);
                continue;
            }
...
fn start_clickhouse(ch: &ClickHouse, __strict: bool) -> Result<()> {
    ...
    cmd.args([
        "-p",
        &format!("{}:{}", ch.http_port, ch.container_http_port),
    ]);
    cmd.args([
        "-p",
        &format!("{}:{}", ch.native_port, ch.container_native_port),
    ]);
...
fn start_postgres_extra(
    pg: &PostgresExtra,
    pool_conn: Option<&PoolConnection>,
    __strict: bool,
) -> Result<()> {
...
fn start_samba(smb: &Samba, __strict: bool) -> Result<()> {
    ...
    cmd.args([
        "-s",
        &format!("{};/mnt/disk1;yes;no;no;{}", smb.share_name, smb.user),
    ]);
...
fn start_big_paragon(bp: &BigParagon, __strict: bool, no_rebuild: bool) -> Result<()> {
    ...
    if no_rebuild {
        if check_image_exists(image_name) {
            ...
            anyhow::bail!(
                "Image {} does not exist. Cannot start without building. Remove --no-rebuild flag to build the image.",
                image_name
            );
        }
    } else {
